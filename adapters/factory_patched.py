...
PATCHES APPLIED:
  ...
  ✅  FIX-2 (Forensic Audit): Cloud / unknown models are now routable.
           Previously the factory only knew about 'qwen2.5-vl'. Any other
           model name raised ModelNotRecognizedException, making PureLLMWrapper
           dead code and breaking the "supports cloud also" design claim.

           New routing logic (in priority order):
             1. Explicit _LOCAL_REGISTRY entry → use registered local adapter.
             2. Model name in _CLOUD_REGISTRY → use PureLLMWrapper with that name.
             3. Unknown model → raise ModelNotRecognizedException with clear message.

           PureLLMWrapper is lazy-imported only when a cloud model is requested,
           so Ollama-only boots never touch cloud SDK code paths.

  ✅  §R3 (prior): get_action() classmethod uses module-level adapter cache
           keyed by model_name — one instance per model, lock-protected.

  ✅  §DEF-1 (prior): 'ollama/' prefix format supported and stripped before
           registry lookup.

  ✅  EVO-3 (Forensic Audit): On AdapterClass instantiation failure the
           partial cache entry is guaranteed to be absent (try/finally cleanup).

EXTENSION CONTRACT:
  To add a new LOCAL model (Ollama, llama.cpp, etc.):
    Add an entry to _LOCAL_REGISTRY:
      "my-model-name": "adapters.my_adapter.MyAdapterClass"

  To add a new CLOUD model (OpenAI, Anthropic, Gemini, etc.):
    Add the base model name to _CLOUD_REGISTRY.
    PureLLMWrapper will resolve it via its own internal registry.
