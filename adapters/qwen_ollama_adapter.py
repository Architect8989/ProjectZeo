...
PATCHES APPLIED (Audit Fixes):
  ...
  ✅  §R4  (was §qwen-7): ollama client read timeout raised from 25s to 120s.
           CPU inference on Qwen2.5-VL 7B takes 40–90s on consumer hardware.
           The prior 25s timeout caused spurious failures on every action cycle.

  ✅  §R9  (was §qwen-8): Current task objective is now injected into the
           per-action user prompt so the LLM retains goal context throughout
           execution, not just at system-prompt time.

  ✅  §1.3 (original): ollama.Client response accessed via attribute shim —
           handles both ollama ≥0.2 object and legacy dict shapes.

  ✅  §DEF-2: _resolve_click_coordinates() now accepts coordinate-based clicks
           that lack a 'text' key. Previously these were silently dropped
           (fail-closed) even when the LLM provided explicit x/y coordinates
           for elements it identified visually. On raw-OS or sparse-OCR
           screens (terminals, installer dialogs) this caused valid actions to
           vanish without any error signal. Fix: if a click op has no 'text'
           but has numeric 'x' and 'y', pass it through unchanged. The
           fail-closed guard is retained for clicks with neither text nor coords.

  ✅  FIX-5 (Audit): _get_ocr_reader() is now non-fatal.
           On a raw OS with no internet access, the EasyOCR model download
           (~150 MB) will fail. Previously this raised RuntimeError which
           crashed the execution loop on every action cycle.
           Fix: catch all initialisation failures, set _OCR_UNAVAILABLE=True,
           log a warning, and return None. _resolve_click_coordinates() checks
           the None return and falls back to coordinate-only mode — clicks with
           explicit x/y still work; text-based clicks are dropped (fail-closed).
