...
class IntentListener:
    ...
    def __init__(self, mode_controller, snapshot_provider):
        ...
        # GAP-4 FIX: In-memory task queue for sequential intent processing.
        # The intent file can be written as a JSON array of intent strings, e.g.:
        #   ["Open Firefox", "Search for Python docs", "Save the first result"]
        # These are enqueued atomically and processed one by one.
        # Single-string format (existing behaviour) is still fully supported.
        # The queue is a deque so that new items added mid-session (via a new
        # intent file write) are appended to the right and drained left-to-right.
    ...
    def _listen_loop(self):
        # RT-A6 FIX: exponential backoff on consecutive arm failures.
        ...
        while self._running:
            try:
                ...
                # GAP-4 FIX: Drain the in-memory queue first before peeking the
                # intent file.  This ensures that tasks submitted as a JSON array
                # are processed sequentially without requiring the operator to
                # repeatedly write the intent file between tasks.
                intent: Optional[str] = None
                intent_file_path: Optional[str] = None

                if self._task_queue:
                    intent = self._task_queue.popleft()
                    # Queue intents come from the file that was already consumed.
                else:
                    intent, intent_file_path = self._peek_intent()

                    # GAP-4 FIX: If the intent is a JSON array, enqueue all items
                    # and pop the first one to arm immediately.
                    if intent and intent.lstrip().startswith("["):
                        try:
                            ...
                            if (
                                ...
                                # Injection-scan each queued intent individually
                                clean = [
                                ...
                                if len(clean) < len(candidates):
                                    print(
                                        f"[IntentListener] GAP-4: Dropped "
                                ...
                                if clean:
                                    print(
                                        f"[IntentListener] GAP-4: Queuing "
                                ...
                                # Not a valid string-array — treat as plain text
                                pass
                        except (json.JSONDecodeError, ValueError):
                            # Not valid JSON — treat as plain intent text
                            pass
                ...
                if arm_failure_reason is not None:
                    ...
                    # Put the intent BACK at the front of the queue so it is
                    # retried on the next cycle without re-reading the file.
                ...
                # GAP-4: Print queue depth so operator knows how many tasks remain
                if self._task_queue:
                    print(
                        f"[IntentListener] GAP-4: Armed intent {intent[:60]!r}. "
