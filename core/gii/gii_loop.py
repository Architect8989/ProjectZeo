...
class GIIGoalDirectedLoop:
    ...
    def __init__(
        ...
        *,
        ...
        # ── UI-AGILE Grounding Trainer (Blueprint §9.5) ────────────────────────
        # Wired here so every click/navigate action can record grounding data.
        # Gracefully absent if not installed.
        try:
        ...
        while self._iteration < self._max_iterations:
            ...
            if action.get("operation") == _DONE_OP:
                ...
                if self._grounding_trainer is not None:
                    try:
                        self._grounding_trainer.on_episode_complete(success=True)
                    except Exception:
                        pass
            ...
            try:
                ...
                # For click/navigate actions, record grounding data so the
                # grounding trainer can compute reward and accumulate training
                # triplets for nightly fine-tuning.
                if self._grounding_trainer is not None:
                    ...
                    if _op in ("click", "navigate", "tap"):
                        try:
                            ...
                            _logger.debug(
                                "[GIILoop] Grounding record failed (non-fatal): %s", _gt_exc
                            )
