...
class ExecutionPlanner:
    ...
    # -------------------------------------------------------
    # DANGEROUS_PATTERNS — AUDIT-CORRECTED
    #
    # REMOVED patterns that destroyed real-world autonomy:
    #   ";", "&&", "||", "|", ">", "<"     — blocked all shell pipelines,
    #                                          chaining, and redirection
    #   "sudo"                              — blocked privilege escalation
    #                                          required for apt/dnf/brew installs
    #   "bash -c", "sh -c"                  — blocked sub-shells needed by
    #                                          many install scripts
    #   "python -c", "python3 -c"           — blocked inline python used by
    #                                          venv bootstraps etc.
    #   "eval", "exec"                      — blocked dynamic execution that
    #                                          many package managers rely on
    #
    # RETAINED patterns that represent genuine destructive risk:
    #   rm -rf, dd, mkfs, format, chmod 777 — irreversible disk / permission damage
    #   nc, netcat                          — exfiltration primitives
    #   crontab                             — persistent backdoor risk
    #   base64 -d                           — payload decode/exec pattern
    #   $( )                               — command substitution (injection risk)
    #   at <time>                           — deferred execution backdoor
    # -------------------------------------------------------
    DANGEROUS_PATTERNS = [
        r"\brm\s+-rf\b",
        r"\bdd\b",
