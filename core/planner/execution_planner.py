...
class ExecutionPlanner:
    ...
    def _call_llm_sync(self, prompt: str) -> str:
        ...
        DEF-6 FIX: The prior implementation used a bare `except RuntimeError`
        to detect "no running loop", which also silently swallowed RuntimeErrors
        raised by the LLM callable itself inside _call_llm_async (e.g. timeout,
        connection refused), turning them into misleading PlanningError("Invalid
        JSON from LLM") messages.

        Fix: inspect the exception message explicitly. Only the canonical
        asyncio "no running event loop" RuntimeError triggers the asyncio.run()
        path. Any other RuntimeError is re-raised immediately so callers see the
        real failure cause.
        """
        try:
            asyncio.get_running_loop()
            # If we reach here, we ARE inside a running loop — that's the error.
            raise PlanningError(
                "ExecutionPlanner._call_llm_sync() must not be called "
                "inside a running event loop. Use _call_llm_async() instead."
            )
        except RuntimeError as exc:
            # asyncio raises RuntimeError("no running event loop") when there is
            # no current loop.  Any other RuntimeError is a real error — re-raise.
