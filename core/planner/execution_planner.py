...
class ExecutionPlanner:
    ...
    def _expand_goal(self, goal: str, *, include_screen_context: bool = True) -> List[Dict[str, Any]]:
        """
        P1 FIX: include_screen_context is now opt-in (default True for backward
        compatibility, but callers should pass False for text-only goals).

        Previously _read_screen_context() was called unconditionally here,
        injecting up to MAX_SCREEN_CHARS=2000 chars of live world-graph entities
        into every planning prompt — even for goals like "install Node.js" where
        the current screen is irrelevant. This was noise that degraded plan
        quality and risked leaking sensitive visible content (passwords, API keys
        visible on screen at planning time).

        Screen context is now gated. Callers that determine the goal is
        screen-dependent (e.g. a UI automation goal) should pass
        include_screen_context=True explicitly. Text-only goals (command
        execution, tool installation, file creation) should pass False.
        """
