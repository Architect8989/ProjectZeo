...
class ExecutionPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        # SI-3 / Fix-2 FIX: Build a frozenset of all commands from
        # COMMON_INSTALL_COMMANDS that are trusted installer sources.
        # These commands are generated by the system itself (not by user
        # input or LLM hallucination) and must not be silently dropped by
        # the dangerous-patterns filter, which would otherwise block Docker,
        # Rust, Bun, and GitHub CLI installation on Linux.
        #
        # The allowlist is checked BEFORE the pattern scan in
        # _validate_and_normalise_step(): a command that exactly matches a
        # trusted installer command bypasses the pattern filter entirely.
    ...
    def _build_trusted_installer_allowlist() -> frozenset:
        """
        SI-3 FIX: Build a frozenset of all commands from COMMON_INSTALL_COMMANDS
        that are known-safe installer sources.

        These commands may match DANGEROUS_PATTERNS (e.g. curl|sh for Docker,
        Rust, Bun) but are generated by the system's own installer data, NOT
        by user input or LLM hallucination. They must not be silently dropped
        by _validate_and_normalise_step().

        Returns an empty frozenset if the import fails (graceful degradation).
        """
        try:
    ...
    def _call_llm_text(self, prompt: str, *, max_retries: int = 2) -> str:
        ...
        # RB-1 CRITICAL FIX: When _text_model_is_vision_fallback=True the
        # original code called self._llm_call(messages, None, "text_planning_fallback").
        # self._llm_call is run.py's _make_llm_callable() wrapper which ALWAYS
        # returns a Python list of operation dicts (e.g. [{"operation":"click",...}]).
        # str(list) produces Python repr with single-quoted keys — NOT valid JSON.
        # _parse_step_array() then calls json.loads() on this repr and raises
        # JSONDecodeError, so planning fails entirely on every call.
        #
        # Fix: bypass the vision adapter completely and call ollama_client.chat()
        # directly with the vision model name.  This is structurally identical to
        # the non-fallback path below (same client, same options) — the only
        # difference is the model name (self._model_name instead of
        # self._text_model_name, which are equal in fallback mode anyway).
        # The direct Ollama call returns raw LLM text, which is what we need.
        if self._text_model_is_vision_fallback:
            if client is None:
                raise PlanningError(
    ...
    def _validate_and_normalise_step(
        ...
        # SI-3 FIX: Check trusted installer allowlist BEFORE dangerous pattern
        # scanning.  Commands that originate from COMMON_INSTALL_COMMANDS are
        # system-generated (not user/LLM input) and must not be blocked.
        # An exact match against the pre-built frozenset bypasses all patterns.
