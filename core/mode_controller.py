...
class ModeController:
    ...
    def check_armed_timeout(self) -> None:
        ...
        with self._lock:
            ...
            if elapsed > self.MAX_ARMED_SECONDS:
                # Force reset to OBSERVER — does not raise internally
                ...
                self._commit_transition(
                    SystemMode.OBSERVER,
                    f"armed_timeout_recovery (elapsed={elapsed:.1f}s)",
                    True,  # forced=True
                )

        # Raise OUTSIDE the lock to avoid holding lock during exception propagation
        if self._mode is SystemMode.OBSERVER and self._failure_reason == "armed_timeout":
