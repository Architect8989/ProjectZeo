...
class ObserverCore:
    ...
    # H4 FIX: Raised from 150.0s to 300.0s to match main.py WARMUP_TIMEOUT_SECONDS.
    #
    # Root cause: ObserverCore.__init__ starts the grace clock immediately.
    # main.py allows up to 300s for warmup (WARMUP_TIMEOUT_SECONDS=300, env-
    # configurable).  With the old 150s grace, the observer declared blindness
    # at second 150 while the warmup loop was still waiting for frames on a
    # CPU-only host (3 frames × 90s inference = 270s).  Every CPU deployment
    # hit "Observer blind: no initial perception" and aborted before any task
    # could be armed.
    #
