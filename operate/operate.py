...
PATCH AUDIT FIXES:
  ...
  ⚠️  §1.11: InputArbitrator.evaluate() returning WAIT raised
            RuntimeError('REPLAN_REQUIRED') — semantically wrong.
            A WAIT event means a human is active; we should pause and retry,
            NOT trigger a full replan which discards the current plan.
            FIX: WAIT now sleeps for WAIT_RETRY_SECONDS and continues the loop.

  ⚠️  §1.11: `command` operation called os_backend.run_command() but stdout/stderr
            were never captured or fed back to the LLM/world-graph.
            The LLM planned blindly — it could not adapt based on command output.
            FIX: run_command() return value is now inspected; stdout+stderr are
            stored in a bounded execution_log dict and ingested into world_graph
            so subsequent planning steps have terminal context.

  ✅  All existing correct behaviours preserved:
        - BeliefState / ActionRanker / regret tracking
        - MAX_STAGNANT_ITERS=12 → REPLAN_REQUIRED
...
def _execute_autonomous_loop(
    *,
    ...
    while iteration < max_iterations:
        ...
        if observer:
            try:
                ...
                if world_graph and isinstance(perception_snapshot, dict):
                    ...
                    # so world_graph (and subsequently the LLM) can see terminal state
        ...
        # PATCH §1.11: WAIT → pause and retry, NOT REPLAN_REQUIRED
        if authority == AuthorityDecision.WAIT:
            wait_retries = 0
            while wait_retries < MAX_WAIT_RETRIES:
                time.sleep(WAIT_RETRY_SECONDS)
                wait_retries += 1
                # Re-evaluate authority after waiting
                authority = input_arbitrator.evaluate(
                    input_event_ts=time.monotonic(),
                    high_risk=is_high_risk,
                    soc_confident=belief.environment_stability > 0.7,
                ...
                # Still not CONTINUE after max retries → escalate to replan
                journal.record({"event": "wait_timeout_replan"})
                raise RuntimeError("REPLAN_REQUIRED")

        if authority != AuthorityDecision.CONTINUE:
            # Any unrecognised decision (future enum additions) → replan
            raise RuntimeError("REPLAN_REQUIRED")
...
def _execute_decision(
    *,
    ...
    if operation == "command":
        ...
        # PATCH §1.11: return the result dict from run_command (stdout/stderr/returncode)
