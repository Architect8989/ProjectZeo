...
PATCHES APPLIED (Audit Fixes):
  ...
  ✅  §R1  (CRITICAL): _execute_decision() now handles 'file_create' and
           'verify' operations.  Previously both StepType.FILE_CREATION and
           StepType.VERIFICATION steps raised TASK_FAILED:unsupported_operation
           even though the planner schema legitimately produces them.
           FIX:
             - file_create  → os_backend.write_file(path, content)
             - verify       → os_backend.run_command(command) if present,
                              else no-op with success result

  ✅  §R6  (was §stagnation): Stagnation limit is now step-type-aware.
           UI steps: MAX_STAGNANT_ITERS_UI  = 12  (fast feedback expected)
           Command/install steps: MAX_STAGNANT_ITERS_COMMAND = 120
           A slow download (apt-get, npm install) blocks the subprocess for
           minutes — the outer loop sees no progress event and was triggering
           REPLAN_REQUIRED after ~3 seconds.

  ✅  §1.11 (original): WAIT → retry loop (not REPLAN_REQUIRED).
  ✅  §1.11 (original): run_command() return value captured in execution_log
           and merged into world_graph perception for LLM context.

All existing correct behaviours preserved:
  - BeliefState / ActionRanker / regret tracking
  - MAX_STAGNANT_ITERS_UI=12 for UI steps
  - ABORT → AuthorityAbortError
  - action_timeout(30) per action
  - StepVerifier + progress convergence detection
  - AuthorityDecision.RELEASE treated as WAIT (YIELD alias)
"""
...
# PATCH §1.11: WAIT should pause and retry, not replan
WAIT_RETRY_SECONDS = 0.5
MAX_WAIT_RETRIES = 10  # 5s total wait before giving up and replanning
...
def _execute_autonomous_loop(
    *,
    ...
    while iteration < max_iterations:
        ...
        # PATCH §R6: determine stagnation limit based on step type
        ...
        if observer:
            try:
                ...
                if world_graph and isinstance(perception_snapshot, dict):
                    # PATCH §1.11: merge command output from log into perception
        ...
        # PATCH §1.11: WAIT and RELEASE (YIELD alias) → pause and retry
        if authority in (AuthorityDecision.WAIT, AuthorityDecision.RELEASE,
                         AuthorityDecision.YIELD if hasattr(AuthorityDecision, "YIELD") else None):
            wait_retries = 0
        ...
        try:
            ...
            # PATCH §1.11: capture command output and store in execution_log
...
def _execute_decision(
    *,
    ...
    # PATCH §R1: file_create — was missing, caused TASK_FAILED on every file step
    ...
    # PATCH §R1: verify — was missing, caused TASK_FAILED on every verification step
    if operation == "verify":
        command = decision.get("command")
        if isinstance(command, str) and command.strip():
            result = os_backend.run_command(command.strip())
            return result
        # No command — treat as visual verification pass-through
