"""
main.py — Kernel orchestrator
==============================
PATCHES APPLIED:

  ✅  FIX-4 (Forensic Audit): warmup_deadline extended from 8s → 150s.
           On CPU-only hardware, Qwen2.5-VL takes 40–90s per inference.
           The prior 8s window meant zero frames would arrive before the kernel
           started accepting tasks, leaving the planner with an empty world graph.
           150s matches ObserverCore.STARTUP_GRACE_SECONDS (updated in that file
           for the same reason). On GPU hardware the warmup exits early via
           the stable_frames check — no performance regression on fast machines.

  ✅  §R5  (prior): Environment fingerprint refreshed after each successful
           task and before each replan — newly installed tools are visible.

  ✅  §R8  (prior): check_armed_timeout() called every heartbeat — prevents
           infinite ARMED stall when begin_planning() is never called.

  ✅  §main-7 (prior): VisionUnavailableError / ObserverUnavailableError
           from begin_planning() retried up to HEALTH_RETRY_MAX times before
           treating as fatal.

  ✅  §DEF-4 (prior): restore_snapshot() runs under a 60s daemon-thread
           timeout guard — kernel cannot hang indefinitely in RESTORING.

  ✅  §1.5  (prior): HEARTBEAT_INTERVAL reduced from 2.0s to 0.25s.

  ✅  GAP-3 (prior): Planner executor shutdown before each replacement.

  ✅  EVO-4 (Forensic Audit): ObserverBlindnessError now attempts one
           vision runtime restart before breaking the main loop.

MODEL INDEPENDENCE:
  main.py is fully provider-agnostic. It receives llm_callable from run.py
  (which built it from whatever adapter the user requested) and injects it
  into ModeController. No model name, SDK, or provider appears here.
"""
